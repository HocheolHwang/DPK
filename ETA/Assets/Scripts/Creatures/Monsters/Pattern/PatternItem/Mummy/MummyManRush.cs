...
public class MummyManRush : Pattern
{
    ...
    public override IEnumerator StartPatternCast()
    {
        ...
        _controller.GetComponent<NavMeshAgent>().speed = CalcSpeedFromDestTime(destToTarget);

        HitBox hitbox = Managers.Resource.Instantiate("Skill/HitBoxRect").GetComponent<HitBox>();
        hitbox.SetUp(transform, _attackDamage + _patternDmg / 3, -1, false, RushTime);
        hitbox.transform.localScale = _patternRange;
        hitbox.transform.SetParent(_controller.transform);
        hitbox.transform.position = Pos;

        // EFFECT
        ParticleSystem rushingPS = Managers.Effect.Play(Define.Effect.Mummy_Rushing, RushTime, transform);
